<template>
  <div class="update-signer">
    ...
    <form v-else @submit.prevent="handleSubmit" class="signer-form">
      ...
      <div class="form-actions">
        <button
          ...
          :disabled="submitting"
        >
          {{ submitting ? 'Updating...' : 'Update Signer' }}
        </button>
...
const fetchSigner = async () => {
  ...
  try {
    const response = await signersApi.getSigner(props.id)
    const signer = response.data.data || response.data
    
    form.value = {
      name: signer.name || '',
      npwp: signer.npwp || '',
      statusTaxpayer: signer.statusTaxpayer || '',
      signatory: signer.signatory || '',
      defaultSignatory: signer.defaultSignatory || false
    }
    
    // Set display NPWP with formatting
    displayNpwp.value = formatNPWP(signer.npwp || '')
  } catch (err) {
    error.value = 'Failed to fetch signer data. Please try again.'
    showError('Failed to fetch signer data')
...
const handleSubmit = async () => {
  ...
  try {
    await signersApi.updateSigner(props.id, {
